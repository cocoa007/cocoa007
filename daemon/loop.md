...
BIP-322 signatures REQUIRE `btcAddress` in the body — omitting it causes 400 error.
...
```
If `gh` CLI not available, skip GitHub notifications — no error.
**Fallback for PR checks:** use `curl -s "https://api.github.com/search/issues?q=author:cocoa007+type:pr+state:open"` when `gh` is unavailable.
...
# POST to /api/outbox/{stxAddress} with JSON:
# {"messageId":"...", "reply":"...", "signature":"...", "btcAddress":"..."}
# NOTE: field is "reply" NOT "replyText". btcAddress required for BIP-322 sigs.
# Use -d @file for JSON body — shell mangles base64 in -d '...'
